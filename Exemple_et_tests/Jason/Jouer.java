...
public class Jouer extends Scene{
	...
	private int[][] grilleJoueur = new int[10][10]; //Grille personnelle du joueur
	private int[][] grilleVisuJoueur = new int[10][10]; //Servira au joueur pour voir la grille initiale de l'ordi
	private int[][] grilleOrdi = new int[10][10]; //Grille personnelle de l'ordi
	private int[][] grilleVisuOrdi = new int[10][10]; //Servira à l'ordi pour voir la grille initiale du joueur
	...
	*/
	...
	public void remplissage(){
		...
		try //On ouvre le fichier
		{
			...
			if(this.mode == 2){ 
				while(scan.hasNextInt()){
					for(int i=0;i<10;i++){
						for(int j=0;j<10;j++){
							if(this.isJoueur == true){
								this.grilleJoueur[i][j] = scan.nextInt();
								this.grilleVisuOrdi[i][j] = this.grilleJoueur[i][j];
							}
							else{
								this.grilleOrdi[i][j] = scan.nextInt();
								this.grilleVisuJoueur[i][j] = this.grilleOrdi[i][j];
							}
						}
					}
				}
			...
			//On remplit entièrement toutes les grilles avec de l'eau (valeur 0)
				...
				for(int i=0;i<10;i++){
					for(int j=0;j<10;j++){
						this.grilleJoueur[this.x][this.y] = 0;
						this.grilleVisuJoueur[this.x][this.y] = 0;
						this.grilleOrdi[this.x][this.y] = 0;
						this.grilleVisuOrdi[this.x][this.y] = 0;
					}
				}
			}
		}
	...
	public boolean autorisationPlacementJoueur(){
		...
		if(this.grilleJoueur[this.x][this.y] == 0){ 
	...
	public boolean autorisationPlacementOrdi(Coordonnees c, int t, int s){
		...
		if(this.grilleOrdi[this.x][this.y] == 0){
			switch(this.sensBateau){
				case 1: //Sens - Haut
					if((this.y-this.tailleBateau)>-1){
						while(taille<this.tailleBateau){
